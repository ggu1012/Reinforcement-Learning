...
class AGENT:
    def __init__(self, env, tdc = False, is_upload=False):
        ...
        if is_upload:  # Test
            ...
            self.policy = np.zeros((HEIGHT, WIDTH, len(self.ACTIONS))) + 1.0 / len(
                self.ACTIONS
            )
    ...
    # SARSA
    def TD_Control(
        self, discount=1.0, alpha=0.01, epsilon=0.3, decay_period=20000, decay_rate=0.9,
    ):
        ...
        while episode < TRAINING_EPISODE_NUM:
            done = False
            seqlen = 0
            ...
            while not done: 
                ...
                self.Q_values[i, j, action] += alpha * (
                    reward + discount * self.Q_values[next_i, next_j, next_action] - self.Q_values[i,j,action]
                )   

                # Update policy
                self.policy[i,j,:] = np.zeros((len(ACTIONS))) + epsilon / len(ACTIONS)
                max_idx = np.argmax(self.Q_values[i,j,:])
                self.policy[i,j,max_idx] += 1 - epsilon            
        ...
        np.savez("./result/tdc.npz", Q=self.Q_values, V=self.V_values, PI=self.policy)
            ...
            if not timeout:
                ...
                for [i, j], a, next_state, reward in reversed(history):
                    ...
                    if visited[i][j] > 1:
                        ...
                        self.Q_values[i][j][a] += alpha * (
                            cum_reward - self.Q_values[i][j][a]
                        )
                        ...
                        self.policy[i][j] = (
                            epsilon / len(ACTIONS) * np.ones((len(ACTIONS)))
                        )
